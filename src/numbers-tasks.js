...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const distanceVector1 = Math.sqrt(x1 * x1 + y1 * y1);
  const disyanceVector2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cosAngle = sumVectors / (distanceVector1 * disyanceVector2);
...
function getSumOfNumbers(x1, x2, x3) {
  if (!Number.isInteger(x1) || !Number.isInteger(x2) || !Number.isInteger(x3)) {
    const x = Math.trunc(x1 * 10) + Math.trunc(x2 * 10) + Math.trunc(x3 * 10);
    return x / 10;
  }
  return x1 + x2 + x3;
}
